...
fileInput.addEventListener("change", async () => {
  ...
  for (const [i, file] of files.entries()) {
    ...
    await file
      ...
      .pipeTo(controller.add({ name: file.name, size: file.size, type: "file" }));
  }
  controller.finalize();
  ...
  // Show final result
  output.textContent =
    formatProgress(files, totalSize, totalSize, files.length, 0) +
    ...
    files
      .map((file, i) =>
        i < currentIndex
          ...
          : i === currentIndex
            ? `- ⏳ ${file.name} (${formatSize(file.size)}) ${formatPercent(fileProcessed, file.size)}%\n`
            : `- ⬜ ${file.name} (${formatSize(file.size)})\n`,
      )
      .join("") +
